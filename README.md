# Project QuIST

## Overview
Project QuIST (Quickly Integrated Software Toolkit) is the primary software repository for the Miami Beach Bots' 2027 FRC robot. This project is implemented in C++ using the WPILib Command-Based framework to provide a robust and scalable architecture for robot control.

## Status
The project is currently in the initial development phase. The codebase follows the 2027 WPILib standards.

## Getting Started

### Prerequisites
- WPILib 2027 Toolchain
- C++17 or later
- CMake / Gradle

### Installation
1. Clone the repository:
   ```bash
   git clone https://github.com/MiamiBeachBots/Project-QuIST.git
   ```
2. Navigate to the project directory:
   ```bash
   cd Project-QuIST
   ```
3. Build the project:
   ```bash
   ./gradlew build
   ```

## Documentation
Additional documentation can be found in the `docs/` directory:
- [Contribution Guide](docs/Contribguide.md)
- [Style Guide](docs/styleguide.md)
- [Commit Guide](docs/commitguide.md)
- [Deployment Guide](docs/deploy-guide.md)
- [Support and Assistance](docs/Assist.md)

## License
This project is licensed under the GPL-3.0 License. See the [LICENSE](LICENSE) file for details.

## Branch Purpose: standard-oil
Resource management; optimizes memory allocation and CPU cycles for everything.